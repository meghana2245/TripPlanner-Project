  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const now = new Date();
        setTrips(all.filter((t) => new Date(t.endDate) < now).sort((a, b) => new Date(b.endDate) - new Date(a.endDate)));
    ...
    <div className="min-h-screen bg-slate-900">
      ...
      <PageTransition>
        ...
        <div className="relative pt-16 overflow-hidden">
          <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: "url(https://images.unsplash.com/photo-1530789253388-582c481c54b0?w=1920&q=80)" }} />
        ...
        <div className="max-w-4xl mx-auto px-6 py-10 pb-20">
          {loading ? (
            ...
            <div className="text-center py-24 bg-white/5 border border-white/10 border-dashed rounded-2xl">
              ...
              <Link to="/dashboard" className="bg-teal-600 hover:bg-teal-500 active:scale-95 text-white px-6 py-3 rounded-xl font-medium transition-all inline-block">Plan a Trip</Link>
            </div>
          ) : (
            <div className="relative">
              ...
              <div className="space-y-6">
                {trips.map((trip, i) => (
                  <div key={trip._id} className="flex gap-6 animate-slideUp" style={{ animationDelay: `${i * 0.08}s` }}>
                    <div className="flex flex-col items-center shrink-0 mt-1">
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${i === 0 ? "border-teal-500 bg-teal-500/20" : "border-slate-600 bg-slate-800"}`}>
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-xs bg-slate-700 text-slate-400 px-2 py-0.5 rounded-full">Completed</span>
                          </div>
                          <h3 className="text-white font-bold text-xl mb-2 leading-tight">{trip.tripName}</h3>
                          <div className="flex flex-wrap gap-4 text-sm">
                            <span className="flex items-center gap-1.5 text-slate-400"><MapPin className="w-3.5 h-3.5 text-teal-400 shrink-0" />{trip.destination}</span>
                            <span className="flex items-center gap-1.5 text-slate-500"><Calendar className="w-3.5 h-3.5 shrink-0" />{fmt(trip.startDate)} – {fmt(trip.endDate)}</span>
                          </div>
                        </div>
                        <Link to={`/trips/${trip._id}`} className="shrink-0 ml-4 flex items-center gap-1.5 bg-slate-700 hover:bg-teal-600 active:scale-95 text-white text-sm px-4 py-2 rounded-lg transition-all group-hover:bg-teal-600">
